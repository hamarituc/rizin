...
RZ_API bool rz_core_analysis_everything(RzCore *core, bool experimental, char *dh_orig) {
	...
	if (!rz_str_startswith(rz_config_get(core->config, "asm.arch"), "x86")) {
		...
		rz_core_notify_begin(core, "Emulate functions to find computed references (aaef)");
		if (plugin_supports_esil) {
			rz_core_analysis_esil_references_all_functions(core);
		}
		rz_core_notify_done(core, "Emulate functions to find computed references (aaef)");
		rz_core_task_yield(&core->tasks);
	...
	if (rz_config_get_i(core->config, "analysis.autoname")) {
		rz_core_notify_begin(core, "Speculatively constructing a function name "
					   "for fcn.* and sym.func.* functions (aan)");
		rz_core_analysis_autoname_all_fcns(core);
		rz_core_notify_done(core, "Speculatively constructing a function name "
					  "for fcn.* and sym.func.* functions (aan)");
		rz_core_task_yield(&core->tasks);
	}
	if (core->analysis->opt.vars) {
	...
	if (!sdb_isempty(core->analysis->sdb_zigns)) {
		rz_core_notify_begin(core, "Check for zignature from zigns folder (z/)");
		rz_core_cmd0(core, "z/");
		rz_core_notify_done(core, "Check for zignature from zigns folder (z/)");
		rz_core_task_yield(&core->tasks);
	}
	if (plugin_supports_esil) {
		rz_core_notify_begin(core, "Type matching analysis for all functions (aaft)");
		rz_core_analysis_types_propagation(core);
		rz_core_notify_done(core, "Type matching analysis for all functions (aaft)");
		rz_core_task_yield(&core->tasks);
	}

	rz_core_notify_begin(core, "Propagate noreturn information");
	rz_core_analysis_propagate_noreturn(core, UT64_MAX);
	rz_core_notify_done(core, "Propagate noreturn information");
	rz_core_task_yield(&core->tasks);

	// Apply DWARF function information
	Sdb *dwarf_sdb = sdb_ns(core->analysis->sdb, "dwarf", 0);
	if (dwarf_sdb) {
		rz_core_notify_begin(core, "Integrate dwarf function information.");
		rz_analysis_dwarf_integrate_functions(core->analysis, core->flags, dwarf_sdb);
		rz_core_notify_done(core, "Integrate dwarf function information.");
	}
	...
	if (experimental) {
		if (!didAap) {
			rz_core_notify_begin(core, "Finding function preludes");
			(void)rz_core_search_preludes(core, false); // "aap"
			rz_core_notify_done(core, "Finding function preludes");
			rz_core_task_yield(&core->tasks);
		}

		rz_core_notify_begin(core, "Enable constraint types analysis for variables");
		rz_config_set(core->config, "analysis.types.constraint", "true");
		rz_core_notify_done(core, "Enable constraint types analysis for variables");
	}
	...
	if (is_debug) {
		...
		rz_list_foreach (list, iter2, map2) {
			...
			if ((to - from) > MAX_SCAN_SIZE) {
				rz_core_notify_begin("Skipping large region (from 0x%08" PFMT64x " to 0x%08" PFMT64x ")\n", from, to);
				continue;
			}
